...
| ブラウザ駆動のシステムテスト | **develop の工程には無い。** 足すプロジェクトは `tests/system/`（§6）に置き、既定スイートのコマンドに混ぜない（[common/testing.md](../common/testing.md) の「スイートは実行に何を要求するかで分ける」） |
...
| **既定スイート**の住所 | `tests/js/` に対象構成をミラー |
| **システムスイート**の住所 | `tests/system/`（ブラウザ駆動。工程外の opt-in。ツール構成はプロジェクト任せ） |
| テストのファイル名 | `<対象>_test.js` |
| 全件実行（既定スイート） | `node --test "tests/js/**/*_test.js"` |
| スコープ実行 | `node --test --test-name-pattern "F-001" "tests/js/**/*_test.js"` |
| 機能ID タグ | `describe("F-001 <振る舞いの説明>", …)` で機能ID を先頭に置く |

**frontend に結合スイートは置かない。** 実 DB・実サービスへ接続するのは backend の役割で、
frontend 側で実サーバに繋ぎに行くテストはブラウザ有無にかかわらず**システムスイート扱い**にする（既定スイートに混ぜない）。

**既定スイートのコマンドは `tests/js/**` しか拾わない**ので、`tests/system/` を足しても赤緑ループには混入しない。
`tests/js/` 配下にブラウザ依存のテストを置かないことだけ守れば、分離は住所だけで機械的に成立する。
...
- [ ] その契約適合テストを `tests/js/`（既定スイート）に置いたか（`tests/system/` へ出していないか）
