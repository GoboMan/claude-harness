...
住所と実行コマンドはレイヤ側の葉が定める（[frontend/testing.md](../frontend/testing.md) ／ [backend/testing.md](../backend/testing.md)）。
...
API クライアントの契約適合テスト（[frontend/testing.md](../frontend/testing.md)）は、名前の印象に反して**結合スイートではない**。
request 組み立て・response 解釈を純粋関数に割ってあるので外部環境を起動せず、上表の判定基準では既定に落ちる。
