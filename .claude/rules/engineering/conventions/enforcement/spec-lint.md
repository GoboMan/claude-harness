...
- **契約カバレッジ（warn）**: **spec が `fixed`（＝実装に進んでよい）なのに契約ファイルが無い**機能を
  warn で可視化する。実装は契約 `fixed` を前提とするため、契約の**不在**を沈黙させない。
  契約整備前の中間状態（spec を固めてから契約を書くまで）で誤検知しうるため error でなく warn。
...
**spec と契約が両方存在し、かつ `fixed`** であることを要求する。**契約ファイルが「無い」場合も違反**
（契約なしのまま実装が進む事故を断つ。以前は契約欠落を素通りさせる穴があった）。トレーラが無ければ
素通り（**オプトイン**。使わないプロジェクトでは未強制になる旨を明示する）。
