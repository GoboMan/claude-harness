# 📦 crow — コーディング規約（PHP 独自フレームワーク）

> crow で新規システムを作る際の**最低限のコーディングルール**。
> 進め方の基盤は [process.md](../../practices/process.md)（反証駆動・縦切りスライス）に従うこと。
...
- HTML の中のコメントは **`<!-- ～ -->` を使わず**、PHP のコメントを使う（クライアントにコメントを送出しないため）。
...
- ただし：
  - HTML はタグの途中で折り返さない。
  - コメントの `// ------略-----` は、頭のインデントを含めて横幅 80 文字にあわせる。
