...
> **テストの原則（何を・なぜテストするか）は [process.md](../../practices/process.md) が土台。**
...
これは process.md の受け入れ条件 Given-When-Then にそのまま対応する。
...
process.md の「ハッピーパスだけでなく失敗・空・権限・境界を条件に含める」を実装する手段。
...
- process.md の通り、**テスト緑は前提条件であって完成条件ではない**。緑の後にレッドチーム（攻撃）で壊しにいく。

## ✅ テスト着手前チェックリスト

- [ ] 対象の GWT 受け入れ条件（process.md）を先に確認したか
