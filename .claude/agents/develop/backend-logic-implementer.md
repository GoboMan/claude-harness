...
- **framework 固有の記述規約**（例: crow の coding.md＝プロダクションコードの記法）が渡されたらそれに従う。**渡されなければ従うべき framework 規約は無い**（自分でカタログを探しに行くな・捏造しない）。テストの記法規約（例: crow の testing.md）は Red を起票する test-designer が従うもので、あなた（緑にする側）には渡されない。
