...
- **1 機能 1 ファイル。** `x-feature-id` はディレクトリ名の ID 部・spec.md の機能ID と一致させる。`x-spec` で親 SSOT を指す。
- **機能詳細に無い入出力を足さない。** 逆に、機能詳細の入力・出力・状態（error／権限なし／境界を含む）を過不足なく表現する。エラーは発生条件ごとにステータスコード＋`Error` 形で列挙する。
