...
- **あなたが書くのは「外部環境を一切起動しないテスト」だけである。** 3 トラックのいずれも、実 DB・実サービス・ブラウザ・実機を起動しない前提で書け（境界はモックする）。これらを起動するテストは別スイート（別フォルダ）の担当で、下流の赤緑ループに混ぜると Phase4 が回らなくなる。住所と分離方法はテスト規約の葉の「スイート」節に従う。**とりわけ API クライアントの契約適合テストは既定スイートに置く**（純粋関数として書けば外部環境が要らない）。
