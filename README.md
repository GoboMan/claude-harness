...
> **なぜ agents も射影するのか（Cursor は `.claude/agents` も直接読むのに）:** 各 agent は `model: opus/sonnet` を持つが Cursor はこれを解決できない恐れがある。そこで `.cursor/agents/`（名前衝突時に `.claude/agents/` より優先）へ `model: inherit` に正規化した版を置いて上書きする。`tools:` は Cursor が解釈しないが害が無いので残す（read-only オラクル/攻撃の規律は各 agent body の指示で担保される）。
